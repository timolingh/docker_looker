...
ENV UPDATE 191
ENV BUILD 12
ENV SIG 2787e4a523244c269598db4e85c51e0c
...
# Download and copy this manually
#COPY ./jdk-${VERSION}u${UPDATE}-linux-x64.tar.gz .
#RUN cat jdk-${VERSION}u${UPDATE}-linux-x64.tar.gz | tar xz -C /tmp
#RUN ls -l /tmp

#RUN tar xz jdk-${VERSION}u${UPDATE}-linux-x64.tar.gz -C /tmp  && \
#  mkdir -p /usr/lib/jvm && mv /tmp/jdk1.${VERSION}.0_${UPDATE} "${JAVA_HOME}" && \
#  apt-get autoclean && apt-get --purge -y autoremove && \
#  rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
...
RUN \
  curl `cat $APP_HOME/looker_jar_loc.txt` > $APP_HOME/looker.jar

# COPY ./looker-latest.jar $APP_HOME/looker.jar
